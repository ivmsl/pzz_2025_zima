...
export default function EventCreatorComponent({
  user,
  onClose,
  onSubmit,
  userSearchFn,
  ...
  const [doTimeVote, setDoTimeVote] = useState(
    event?.time_poll_enabled ? true : false || false,
  );
  const [doLocationVote, setDoLocationVote] = useState(
    event?.location_poll_enabled ? true : false || false,
  );

  const eventStart =
    !doTimeVote && formData.date && formData.startTime
  ...
  const handleSubmit = (e) => {
    ...
    if (!doTimeVote) {
      ...
      if (formData.startTime.trim() && formData.endTime.trim()) {
        ...
        if (
          ...
          newErrors.endTime =
            "Godzina końcowa musi być późniejsza niż startowa";
        }
      }
    } else {
      ...
      if (!timeVoteRef.current) {
        newErrors.timeVote =
          "Wystąpił błąd podczas tworzenia głosowania nad czasem";
      }
    }
    ...
    if (!doLocationVote) {
      ...
      if (!locationVoteRef.current) {
        newErrors.locationVote =
          "Wystąpił błąd podczas tworzenia głosowania nad miejscem";
      }
    }
    ...
    let formToSubmit = {
      ...
      voteObjects: {
        time: doTimeVote
          ? timeVoteRef.current?.returnVoteDescriptor() || null
          : null,
        location: doLocationVote
          ? locationVoteRef.current?.returnVoteDescriptor() || null
          : null,
        general:
          voteNum > 0
            ? generalVoteRef.current.map((vote) =>
                vote.returnVoteDescriptor(),
              ) || []
            : [],
      },
    };
  ...
  const removeChosenUsers = (user) => {
    handleChange(
      "participants",
      formData.participants.filter((p) => p.id !== user.id),
    );
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-[95vw] max-w-7xl h-[85vh] max-h-[85vh] overflow-x-visible relative flex flex-col">
        ...
        >
          <div className="overflow-y-auto overflow-x-visible flex-1 p-8 pb-6">
            <div className="flex gap-8 overflow-x-visible">
              ...
              <div className="flex-1 pb-4">
                ...
                <div
                  ...
                  style={{ marginBottom: "1rem", marginTop: "1.2rem" }}
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Maksymalna długość nazwy wydarzenia wynosi{" "}
                      {MAX_EVENT_NAME_LENGTH} znaków
                    </p>
                    {errors.name && (
                ...
                <div
                  className="flex gap-4 overflow-visible flex-row"
                  style={{ marginBottom: "1rem" }}
                >
                  <div
                    className="flex items-start gap-3 flex-1 relative overflow-visible"
                    ref={startTimeRef}
                  >
                  ...
                  <div className="flex-col items-center gap-3">
                    <Checkbox
                      checked={doTimeVote}
                      onCheckedChange={(e) => setDoTimeVote(e)}
                ...
                >
                  ...
                  <div className="flex-col items-center gap-3">
                    <Checkbox
                      checked={doLocationVote}
                      onCheckedChange={(e) => setDoLocationVote(e)}
                      className="w-5 h-5 text-gray-500 shrink-0 mt-2"
                    />
                    <p className="text-sm text-gray-500">Vote?</p>
                  </div>
              ...
              <div className="flex-1 space-y-6 pb-4">
                ...
                <SearchUserComponent
                  ...
                  addChosenUsers={addChosenUsers}
                  chosenUsers={formData.participants}
                />
                  ...
                  <div className="flex flex-wrap gap-2">
                    {formData.invitees.map((invitee, index) => (
                      <span
                        ...
                        style={{ marginTop: "1rem" }}
                      >
                        {invitee.username || invitee.email || invitee}{" "}
                        ...
                        <button
                          ...
                          onClick={() => {
                            handleChange(
                              "invitees",
                              formData.invitees.filter((_, i) => i !== index),
                            );
                          }}
                          className="hover:text-blue-900"
                        >
            ...
            >
              {doTimeVote && (
                <GeneralVote
                  user={user}
                  eventId={formData.id}
                  voteObj={event?.voteObjects?.time[0]}
                  type="time"
                  ref={timeVoteRef}
                  disabled={!!event?.id}
                />
              ...
              {doLocationVote && (
                <GeneralVote
                  user={user}
                  eventId={formData.id}
                  voteObj={event?.voteObjects?.location[0]}
                  type="location"
                  ref={locationVoteRef}
                  disabled={!!event?.id}
            ...
            {voteNum > 0 &&
              Array.from({ length: voteNum }).map((_, index) => (
                <GeneralVote
                  user={user}
                  key={index}
                  type="general"
                  eventId={formData.id}
                  ref={(element) => {
                    generalVoteRef.current[index] = element;
                  }}
                  eventStart={eventStart}
                />
              ))}
