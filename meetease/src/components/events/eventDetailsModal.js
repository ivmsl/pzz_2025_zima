...
export default function EventDetailsModal({user, event = defaultEvent, attendees = defaultAttendees, serverActions}) {
    ...
    if (attendees == defaultAttendees) {
        attendees = event.attendees || defaultAttendees
    }
    ...
    const handleEditClick = () => {
        console.log("Editing event", event.id, event, user.id)
    ...
    const handleEditSubmit = async (event) => {
        if (serverActions.handleUpdateEventServerAction) {
            ...
            console.log("Updating event", event.id, event, user.id)
            const { success, error } = await serverActions.handleUpdateEventServerAction(event.id, event, user.id)
        }
                        ...
                        <div className={'flex flex-col gap-4'}>
                            <div className="flex flex-col gap-1 text-lg">
                                <div className={'flex gap-2 items-center'}>
                                    ...
                                    <span>{event.date || "Brak daty"}, {event.time || event.time_start || "Brak czasu"}</span>
                                </div>
                                {event.location && (
                                    <div className={'flex gap-2 items-center'}>
